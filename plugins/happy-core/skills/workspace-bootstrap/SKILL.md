...
1. repo rootと適用範囲の`AGENTS.md` chain、`.github/copilot-instructions.md`、近接docs、技術stack、build/test command、CI、および適用対象の他skillが提供する`AGENTS.fragment.md`を確認する。存在しないsourceは推測で補わない。
...
4. `.github/copilot-instructions.md`がある場合は全文を複製せず、Codexの変更やreviewの判断を実質的に変える高重要度のルールだけを抽出する。既存指示と同義の内容は重複させず、矛盾は優先順位と適用範囲を確認して解消する。
