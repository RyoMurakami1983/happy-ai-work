...
タスクの最初に`$yohaku`を明示して使うと、同じ会話の後続依頼にも継続適用する方針になります。「この回答だけ」で範囲を限定でき、「Yohakuを解除して」で終了できます。文脈の引き継ぎには適用状態とスキルの場所・主要原則を残し、再開後に必要なら読み直します。圧縮後の完全な保持は保証しません。新しい会話で毎回適用したい場合は、下記のhome-bootstrapで任意設定を選べます。会話内の解除と、今後の会話も含む設定変更は別に扱います。
...
Yohakuが選択可能で本文を読める場合は、新しい会話の開始時から適用する設定を希望するか確認します。希望した場合だけ開始時の指示を既存管理領域へ合成し、有効・無効の選択は通常更新で保持します。未導入でも基本方針だけで利用できます。詳しい説明の調整はYohaku本体が担当します。

通常更新の`--yohaku preserve`は既定値です。選択を変えるときは`--yohaku enable`または`--yohaku disable`を付けてdry-runし、差分の承認後に同じ引数でapplyします。未設定と無効化済みは区別されます。旧版の更新scriptでは選択が消える可能性があるため、downgrade時は差分とbackupを確認してください。
