...
Date: 2026-09-26
Status: 設計レビューを反映した実装計画案。実装開始は未承認。
...
- [x] 実装計画をレビュー結果に整合させる
- [ ] 利用者による具体的な実装範囲・開始の承認
- [ ] Slice 1: 相談と実装許可を区別して引き継ぐ
- [ ] Slice 2: 任意のYohaku選択を安全に設定・更新する
- [ ] Slice 3: 設定に従って会話開始時からYohakuを適用する
- [ ] Completion handoff

今回許可されているのは文書作成・レビュー。以下のコード・instruction変更や行動評価は未着手。文書承認を実装開始の許可と解釈しない。
...
- [ADR案0005](../adr/0005-workflow-entry-and-optional-startup.md): 責務配置と既存判断への影響。Proposed。
...
- [ ] B-01: Issue選択の2ターンで実装せず、調査・要求整理を進める。
- [ ] B-02: PRD短縮の理由と許可を確認し、許可済みの同一範囲では再確認せず実装へ渡す。
- [ ] B-03: 複数ACを持つ一つの振る舞いを扱え、構造判断が必要なら設計へ戻す。
- [ ] B-04: 未設定・有効・無効を区別し、dry-run・apply・再適用で利用者の選択と管理外内容を保つ。
- [ ] B-05: 有効化後の新会話で最初にYohakuを読み、継続・解除・利用不可を適切に扱う。
...
公開するのはversion付きcase、固定TARGET、sanitize済みの新規record。生の応答や個人情報を公開せず、旧recordは上書きしない。今回の文書作成ではこれらの評価資産作成・runは行わない。PR #34の未完了評価を遡って完了にしない。
...
```powershell
uv run --python 3.14 python -m unittest discover -s tests -p test_home_bootstrap.py -v
uv run --python 3.14 python -m unittest discover -s tests -p test_work_artifact_policy.py -v
uv run --python 3.14 python -m unittest discover -s tests -p test_evaluation_assets.py -v
```
