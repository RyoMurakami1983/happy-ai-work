...
最新状況（2026-09-15）: Ubuntu Server 26.04でCompose・テスト用HTTP/SQLiteアプリ・SMB共有・ホスト再起動後の共有復帰を検証した。以下の日付付き記録は当時の状態を保持する。実案件での手戻り削減は未検証であり、プレビューを継続する。
...
## 2026-09-14〜15: Ubuntuでの機能・実機検証

利用者が用意したUbuntu Server 26.04.1 LTS（arm64）、Docker、Compose、NASの専用SMBテスト共有で実施した。接続先、認証情報、環境固有スクリプト、生ログは公開せず、利用者管理のローカル記録へ保存した。

| 検証範囲 | 結果 |
| --- | --- |
| Compose | 非rootでのデータアクセス、読取専用bind、保存の永続化、同一設定の再適用、権限不備からの復旧、欠落bind元の拒否、health失敗を伴う更新と復旧、別保存先への復元を確認 |
| テスト用HTTP/SQLiteアプリ | HTTP経由の保存、コンテナ再起動後の保持、SQLite backup APIと整合性確認、失敗したトランザクションのロールバック、修正した移行の再実行、別DBへの復元を確認 |
| 実SMBと非rootコンテナ | 既知データの読取、原本相当領域への書込拒否、限定したbind範囲、専用保存先、再適用、欠落サブパスでの失敗と修正再実行、別フォルダへの復元、後片付けの17項目が成功 |
| 再接続とホスト再起動 | 制御したアンマウント後の自動再接続が成功。利用者による再起動後、boot IDの変化、SSHとDockerの稼働、パスワード再入力なしでのSMB接続を確認。非rootコンテナの17項目を再実行して成功し、試験用コンテナと専用データを削除 |

途中失敗も記録した。HTTP試験の初版はlocalhost公開の確認で停止し、片付け後に設定を修正して再検証した。複数のネットワーク設定を変えたため、単一の原因は断定しない。

再起動後のSMB試験は、既存の判定が`findmnt --target`で返る`autofs`層を共有の不一致として拒否し、書込み前に停止した。再起動後用のスクリプトで対象パスへのアクセス後に実CIFS層を選択するよう修正し、期待する接続元・mount先・種類の照合を維持して再検証した。この観測と既存の共有照合要件を根拠に、共有ストレージ参照文書へ自動マウント時の確認を補った。

Ubuntuでの欠落サブパス試験はSMB通信断の代用ではない。処理中のネットワーク断、NAS停止、NFS、NAS側ACL拒否、実案件アプリの自動起動・起動順序・処理再開は未検証。模擬アプリと実機の成功を、実案件での効果や通常配布への昇格条件の充足とは扱わない。追加の環境試験はここで区切り、次の実作業で有用性を確認する。
